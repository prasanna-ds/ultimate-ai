...
def test_total_case_count_udf(twitter_raw_dataframe):
    df = twitter_raw_dataframe.withColumn("total_case_count", get_total_case_count_udf())
    rows = df.collect()
...
def test_process_tweet_udf(twitter_raw_dataframe):
    ...
    assert len(rows) == 3
    assert rows[0]["content"] == "Gregor then turned to look out the window at the dull weather."
